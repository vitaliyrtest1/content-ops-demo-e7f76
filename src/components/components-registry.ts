...
import { analyticsTagConfigured } from './GoogleAnalytics/context';
import { withAnalytics } from './GoogleAnalytics/with-analytics';
/**
...
export function getComponent(key: string): ComponentType {
    ...
    if (personalizedComponents[key]) component = withPersonalization(component, personalizedComponents[key]);
    if (hasAnalytics(key)) component = withAnalytics(component);
    return component;
}
/**
...
const components = {
    ...
    CloudinaryImageBlock: dynamic(() => import('./blocks/CloudinaryImageBlock')),
...
function hasAnalytics(key: string) {
    return analyticsTagConfigured && key.endsWith('Section') && key !== 'DividerSection';
}
