    ...
    return (
        ...
        >
            {images.map((image, index) => (
                <Image key={index} image={image} data-sb-field-path={`.${index}`} />
            ))}
        </div>
    );
}
...
function Image(props) {
    const { image } = props;
    const modelName = image.__metadata.modelName;
    if (!modelName) {
        throw new Error(`image does not have the 'modelName' property`);
    }
    const ImageComponent = getComponent(modelName);
    if (!ImageComponent) {
        throw new Error(`no component matching the image model name: ${modelName}`);
    }
    return <ImageComponent {...image} {...getDataAttrs(props)} />;
}
