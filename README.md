...
5. Import the Contentful data stored in the `sources/contentful/export.json` file to the new space by running the following command. Replace the `<management_token>` with your Personal Access Token and the `<space_id>` with the new space ID.

   ```shell
   ./sources/contentful/import/import.js <management_token> <space_id>
   ```
...
10. Once you finish updating the code and contents, export the contents back to the `contentful/export.json` file by running the following command. Replace the `<management_token>` with your Personal Access Token and the `<space_id>` with the new space ID.
    ...
    ```shell
    ./sources/contentful/import/export.js <management_token> <space_id>
    ```
